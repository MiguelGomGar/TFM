...
```{r}
clinical_data <- read_parquet("data/intermediate/clinical_data_full.parquet")
head(clinical_data)
```
...
## Multicolinearity
Before diving into complex relationships among features, let' save a subset of the original data with the HATCH score variables:

```{r}
clinical_data |> 
    select(AF_recurrence, hatch_score, age, heart_failure, hypertension, COPD, stroke) |> 
...
Although the waist to height ratio is slightly better as an obesity indicator (check the numeric distribution stratified by AF recurrence), which is also a risk factor for AF recurrence, the `BMI` feature does not have missing values. Therefore, we will drop the `waist_height` feature:
...
write_parquet(
    clinical_data, 
    "data/clean/clinical_data_full.parquet"
    )
