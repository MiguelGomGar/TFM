...
output_dir <- "results/clinical_data_EDA/"
```
...
plot_stratified_missingness(clinical_data)
```
...
```{r}
plot_row_missingness(clinical_data)
```
...
Numeric features distribution stratified by the center variable:
...
clinical_data |> 
  select(-code) |> 
  plot_stratified_numeric_distribution(target_var = "center")
```
...
Categorical features distribution stratified by the center variable:

```{r, fig.width=10, fig.height=16}
clinical_data |> 
  select(-code) |> 
  plot_stratified_categorical_distribution(target_var = "center")
```
...
Since the waist to height ratio is significantly better as an obesity indicator (see the numeric distribution stratified by AF recurrence), which is also a risk factor for AF recurrence, we will drop the `BMI` feature:
...
# Saving the data
Since there are some machine learning models that deal with NA values, we will save the global data set to use it later on.
